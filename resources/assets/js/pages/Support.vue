<template>
    <main-layout :class="[contentClass]" :style="{ paddingTop: `150px` }">
        <v-container fluid>
            <v-layout row wrap>
                <v-breadcrumbs icons divider="navigate_before" light>
                    ...
                    <v-breadcrumbs-item
                            :disabled="true"
                    >
                        <span class="blue-grey--text">پشتیبانی</span>
                    </v-breadcrumbs-item>
                </v-breadcrumbs>
            </v-layout>
            <v-layout row wrap>
                            ...
                            <v-container fill-height fluid>
                                <v-layout fill-height>
                                    <v-flex xs12 text-xs-center>
                                        <p class="headline primary--text">فرم تماس</p>
                                        <v-text-field
                                                light
                                                name="name"
                                        ...
                                        <v-btn light class="white--text" :loading="contactForm.busy"
                                               :disabled="errors.any()" block
                                               :class="{accent: !contactForm.busy, error: contactForm.busy}"
                                               @click="submit">ارسال
                                            <v-icon left style="transform: rotate(180deg)">send</v-icon>
                                        </v-btn>
                                    </v-flex>
                                </v-layout>
                            </v-container>
                            ...
                            <v-container fill-height fluid>
                                <v-layout fill-height>
                                    <v-flex xs12 text-xs-center align-end flexbox>
                                        <p class="headline primary--text">اطلاعات فروشگاه</p>
                                        <v-list class="grey lighten-4">
                                            <v-list-tile v-for="(value,key,index) in contact_details" :key="key"
                                                         :index="index">
                                                <v-list-tile-content>
                                                    <v-list-tile-title class="title grey--text">
                                                        {{ toProperCase(key) }}: {{ toProperCase(value) }}
                                                    </v-list-tile-title>
                                                </v-list-tile-content>
                                            </v-list-tile>
                                        </v-list>
