...
<modal-layout>
  <v-toolbar class="accent" slot="toolbar">
    ...
    <v-spacer></v-spacer>
    <v-toolbar-title class="text-xs-center primary--text">صفحه مورد نظر یافت نشد</v-toolbar-title>
    <v-spacer></v-spacer>
    <v-toolbar-items>
      <v-btn class="primary--text" flat @click.native="goHome()"><v-icon right dark>home</v-icon></v-btn>
    </v-toolbar-items>
  </v-toolbar>
  <v-card-text style="padding-top:150px; margin-right: 33%">
      <v-container fluid>
        <v-layout row>
        ...
        <v-flex xs12 sm12 md4 offset-md4 lg4 offset-lg4 xl4 offset-xl4>
            <v-card-actions>
            <v-btn @click.native="goHome()" block flat class="info--text info">بازگشت به صفحه اصلی</v-btn>
            <v-btn @click.native="goShop()" block flat class="primary--text primary">ادامه خرید</v-btn>
            </v-card-actions>
         </v-flex>
        </v-layout>
      </v-container>
