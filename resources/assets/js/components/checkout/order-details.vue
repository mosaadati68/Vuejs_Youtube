...
<div>
    <v-layout row>
        ...
        <v-btn class="left" v-else-if="courier && courier.slug ==='credit-card'" @click.native="stripecallback()" :loading="checkOutForm.busy" :disabled="errors.any()"  :class="{primary: !checkOutForm.busy, error: checkOutForm.busy}">پرداخت طریق درگاه پرداخت <v-icon right dark style="transform: rotate(180deg); margin-right: 5px">payment</v-icon></v-btn>
        <v-btn class="left" v-else @click.native="submit()" :loading="checkOutForm.busy" :disabled="errors.any()"  :class="{primary: !checkOutForm.busy, error: checkOutForm.busy}">اتمام خرید <v-icon right dark style="transform: rotate(180deg); margin-right: 5px">send</v-icon></v-btn>
        <v-btn class="left" :disabled="errors.any()" outline color="primary" @click.native="back()">بازگشت</v-btn>
</div>
