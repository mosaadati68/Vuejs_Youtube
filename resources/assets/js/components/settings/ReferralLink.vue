...
<v-tabs-content
    ...
    >
        <v-container>
            <v-layout row wrap>
                <p class="primary--text">آدرس سایت</p>
                <v-flex xs12>
                    <v-alert color="primary" icon="fa-link" value="true">
                        <v-list dense light class="primary">
                            <v-link  link-color="white" :dark="false"  :title="`آدرس فروشگاه انلاین : ${href}`" :href="href"></v-link>
                        </v-list>
                    </v-alert>
                </v-flex>
                <v-flex xs12>
                ...
                <v-btn block color="primary" @click="updateLink()">
                    بروزرسانی آدرس سایت <v-icon right style="transform: rotate(270deg); margin-right: 10px">fa-send</v-icon>
                </v-btn>
            </v-layout>
        </v-container>
    </v-tabs-content>
