...
<v-container fluid>
        <v-layout row v-if="shipmentForm.courier">
            <v-flex xs12 text-xs-center>
                <p class="subheader primary--text">روش ارسال:</p>
            </v-flex>
        </v-layout>
        <v-layout row v-if="shipmentForm.courier">
        ...
        <v-layout row>
            <v-flex xs12 text-xs-center>
                <p class="subheader primary--text">وضعیت ارسال:</p>
            </v-flex>
        </v-layout>
        <v-layout row>
          <v-flex xs12 sm12 md12  lg12  xl12>
        ...
        <v-layout row>
            ...
            <template scope="{ save, cancel }">
                <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn flat color="primary" @click="cancel">انصراف</v-btn>
                <v-btn flat color="primary" @click="save">تایید</v-btn>
                </v-card-actions>
            </template>
            </v-date-picker>
            </v-dialog>
        ...
        <v-layout row>
            <v-flex xs12 sm12 md12  lg12  xl12>
            <v-checkbox :disabled="!hasRole('admin')" v-bind:label="`ارسال`" v-model="shipmentForm.sent" light></v-checkbox>
          </v-flex>
        </v-layout>
        <v-layout row>
            <v-flex xs12 sm12 md12  lg12  xl12>
         ...
         <v-layout row>
            <v-flex xs12 sm12 md12  lg12  xl12>
            <v-checkbox :disabled="!can('edit_order') || !hasRole('admin')" v-bind:label="`دریافت`" v-model="shipmentForm.received" light></v-checkbox>
          </v-flex>
        </v-layout>
        <v-btn light color="primary" :loading="shipmentForm.busy" :disabled="errors.any()"  @click.native="submit()" :class="{primary: !shipmentForm.busy, error: shipmentForm.busy}" class="white--text left">بروزرسانی</v-btn>
</v-container>
</template>
