...
<v-container fluid>
  <v-layout row v-if="paymentForm.gateway">
    <v-flex xs12 text-xs-center>
        <p class="subheader primary--text">اطلاعات درگاه پرداخت</p>
    </v-flex>
  </v-layout>
  <v-layout row v-if="paymentForm.gateway">
  ...
  <v-layout row>
    <v-flex xs12 text-xs-center>
        <p class="subheader primary--text">اطلاعات پرداخت</p>
    </v-flex>
  </v-layout>
    ...
    <v-date-picker v-model="paymentForm.date_paid" scrollable actions light>
      <template scope="{ save, cancel }">
        <v-card-actions>
          ...
          <v-btn flat color="primary" @click="save">تایید اطلاعات</v-btn>
        </v-card-actions>
      </template>
    </v-date-picker>
    </v-dialog>
  ...
  <v-btn light color="primary" :loading="paymentForm.busy" :disabled="errors.any()"  @click.native="submit()" :class="{primary: !paymentForm.busy, error: paymentForm.busy}" class="white--text left">بروزرسانی</v-btn>
</v-container>
</template>
