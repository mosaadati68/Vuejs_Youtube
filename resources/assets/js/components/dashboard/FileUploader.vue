...
<v-container fluid>
    ...
    <v-layout row wrap v-if="!isOption">
        <v-data-table
                ...
                :items="files"
                :light="true"
                no-data-text="برای آپلود فایل بر روی دکمه آپلود فایل ها کلیک نمایید."
            ...
            <template slot="items" scope="props">
                ...
                <td class="title text-xs-left primary--text">
                    ...
                    <span v-else>تصویر موجود نمی باشد</span>
                </td>
                <td class="title text-xs-left primary--text">
                    <v-edit-dialog
                    large
                    ...
                    <div slot="input" class="mt-3 text-xs-center title primary--text">نام فایل</div>
                ...
                <td v-else-if="props.item.active" class="title text-xs-left primary--text">فعال</td>
    ...
    <v-layout row wrap v-if="!isOption">
        <v-spacer></v-spacer>
        <v-btn color="teal lighten-2" v-show="files.length > 0" @click.native="$refs.upload.active = true">شروع آپلود <v-icon right style="transform: rotate(180deg);margin-right: 10px">play_arrow</v-icon></v-btn>
        <v-btn color="error" v-show="$refs.upload && $refs.upload.active" @click.native="$refs.upload.active = false">توفق آپلود <v-icon right style="margin-right: 10px">stop</v-icon></v-btn>
        <v-btn color="red lighten-2" v-if="$refs.upload && !$refs.upload.active && files.length > 0" @click.native="files = []">حذف همه فایل ها <v-icon right style="margin-right: 10px">fa-times</v-icon></v-btn>
        <v-spacer></v-spacer>
    </v-layout>
    <v-layout row wrap v-if="isOption">
        <v-flex xs12>
...
export default {
    ...
    data () {
        return {
            th: [
                { text: 'تصویر', value: 'thumb', align: 'left', sortable: true },
                { text: 'نام فایل', value: 'name', align: 'left', sortable: true },
                { text: 'حجم فایل', value: 'size', align: 'left', sortable: true },
                { text: 'پیشرفت', value: 'progress', align: 'left', sortable: true },
                { text: 'وضعیت', value: 'speed', align: 'left', sortable: true },
