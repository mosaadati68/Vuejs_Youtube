...
const actions = {
    ...
    async register ({ commit, dispatch }, form) {
        ...
        try {
            ...
            form.busy = false
            vm.$popup({ message: 'ثبت نام با موفقیت انجام گردید!', backgroundColor: '#4db6ac', delay: 5, color: '#fffffa' })
    ...
    async login ({ commit, dispatch }, form) {
        ...
        try {
            ...
            await dispatch('fetchMe')
            form.busy = false
            vm.$popup({ message: 'شما با موفقیت به سیستم وارد شدید!', backgroundColor: '#4db6ac', delay: 5, color: '#fffffa' })
            vm.$router.push({ name: 'dashboard' })
        } catch ({errors, message}) {
            form.errors.set(errors)
            form.busy = false
            vm.$popup({ message: 'امکان ورود به سیستم برای شما وجود ندارد!', backgroundColor: '#e57373', delay: 5, color: '#fffffa' })
        }
    },
    ...
    async socialauth ({ commit, dispatch }, form) {
        ...
        try {
            ...
            await dispatch('fetchMe')
            form.busy = false
            vm.$popup({ message: 'شما با موفقیت وارد سیستم شدید!', backgroundColor: '#4db6ac', delay: 5, color: '#fffffa' })
            vm.$router.push({ name: 'dashboard' })
        } catch ({errors, message}) {
            form.errors.set(errors)
            form.busy = false
    ...
    async logout ({ commit }, form) {
        ...
        try {
            ...
            vm.$popup({ message: 'با موفقیت از سیستم خارج شدید!', backgroundColor: '#4db6ac', delay: 5, color: '#fffffa' })
        } catch ({errors, message}) {
            if (errors) {
                console.log(errors)
            }
    ...
    async passwordreset ({ commit, dispatch }, form) {
        ...
        try {
            ...
            await dispatch('fetchMe')
            form.busy = false
            vm.$popup({ message: 'پسورد با موفقیت تغییر کرد!', backgroundColor: '#4db6ac', delay: 5, color: '#fffffa' })
            vm.$router.push({ name: 'dashboard' })
        } catch ({errors, message}) {
            form.errors.set(errors)
            form.busy = false
